...
const csvFilePath = 'C:\\Users\\kokox\\MyProject\\-student-performance-dashboard\\student-dashboard\\src\\scripts\\student-por.csv'; // Adjust the path as needed

fs.createReadStream(csvFilePath)
  .pipe(csvParser())
  .on('data', (row) => {
    // Insert each row into the database
    // Replace column names and placeholders as per your table schema
    ...
    db.run(sql, 
      [row.school, row.sex, row.age, row.address, row.famsize, row.Pstatus, row.Medu, row.Fedu, row.Mjob, row.Fjob, row.reason, row.guardian, row.traveltime, row.studytime, row.failures, row.schoolsup, row.famsup, row.paid, row.activities, row.nursery, row.higher, row.internet, row.romantic, row.famrel, row.freetime, row.goout, row.Dalc, row.Walc, row.health, row.absences, row.G1, row.G2, row.G3],
      (err) => {
        if (err) {
